...
#     # If a valid key was pressed during playback, return it
#     if key in [49, 50, 48]:
#         return key
#     else:
#         # Wait for user input after playback ends
#         print("Playback finished. Waiting for user input...")
#         while True:
#             key = cv2.waitKey(0)  # Wait indefinitely for valid key
#             if key in [49, 50, 48]:  # Valid keys: 1, 2, or 0
...
#         key = cv2.waitKey(30)  # Show each frame for 30ms
#         if key in [49, 50, 48]:  # 1, 2, or 0 key presses
#             break
